...
<script lang=ts>
    ...
    import { CompareDates } from './TimelineProjections';
    ...
    // ----------------------------------------------------------------------
    // ----------------------------------------------------------------------
    const date_to_string_func = (date: Date) => {
        const zero_pad_value = (value: number) => {
            const result = "0" + value.toString();
            return result.substring(result.length - 2);
        }

        _date_string = `${date.getFullYear()}-${zero_pad_value(date.getMonth() + 1)}-${zero_pad_value(date.getDate())}`;
    };

    // ----------------------------------------------------------------------
    const string_to_date_func = (string_date: string) => {
        let result = new Date(
    ...
    $: _date = date;
    $: date_to_string_func(_date);

    $: {
        string_to_date_func(_date_string);
